...
func TestCaptureSupersedeRecordsReason(t *testing.T) {
	...
	if _, err := Do(l, c, Request{
		...
		Date: "2026-08-09", Supersedes: []string{oldStem}, SupersedeReason: reason,
		Body: []byte("## 결정\n"),
	}); err != nil {
		t.Fatal(err)
	}
...
func TestSupersedeWithoutReasonUnchanged(t *testing.T) {
	...
	if _, err := Do(l, c, Request{
		...
		Date: "2026-08-09", Supersedes: []string{oldStem}, Body: []byte("## 결정\n"),
	}); err != nil {
		t.Fatal(err)
	}
	old := readStem(t, l, oldStem)
...
func TestReviewSupersedeReasonLandsOnOldNote(t *testing.T) {
	...
	if _, err := Review(l, ReviewRequest{
		Stem: newStem, Supersedes: []string{oldStem}, SupersedeReason: reason,
	}); err != nil {
		t.Fatal(err)
	}
	old := readStem(t, l, oldStem)
