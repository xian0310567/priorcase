...
type Request struct {
	...
	//
	// 비어도 된다. 강제하면 아직 이 인자를 안 넘기는 호출부(CLI·MCP)에서 뒤집기가
	// 통째로 막힌다 — 사유를 남기라는 압력은 어댑터의 안내문이 넣는 것이 맞다.
...
func Do(l *store.Layout, c *config.Config, r Request) (Result, error) {
	...
	hasOld := false
	if r.Supersedes != "" {
		link, o, err := supersede(l, r.Supersedes, stem, r.SupersedeReason, r.Date)
		if err != nil {
			return Result{}, err
		}
