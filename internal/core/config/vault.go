...
type Vault struct {
	...
	// Shared 는 **여러 사람이 같이 쓰는 볼트**인지다.
	//
	// # 왜 리모트 유무로 추론하지 않는가
	//
	// 개인 볼트도 리모트를 갖는다 — 혼자서 집과 회사 머신을 오가기 때문이다
	// (사업주의 기본 볼트가 정확히 그렇다). 그래서 리모트가 있다는 사실은
	// "누가 쓰는가" 에 대해 아무것도 안 말해 준다. 명시해야 한다.
	//
	// # 무엇이 달라지는가
	//
	// ① **리모트가 필수가 된다.** 개인 볼트는 리모트가 없어도 정상이지만
	//    (그 머신에만 있는 볼트), 공유 볼트에 리모트가 없으면 그 사람의 결정은
	//    아무에게도 안 간다. 그런데 지금까지 그 상태는 조용했다 — doctor 가 말한다.
	//
	// ② **작업 로그를 동기화하지 않는다.** 프로젝트당 파일 하나에 여럿이 같은
	//    자리에 붙이므로 충돌이 일상이 된다(sync/conflict_test.go 의 재현).
	//    자동 병합은 위험하므로 충돌할 파일을 아예 안 만든다.
