...
function fixture(name: string): string {
  return execFileSync(join(__dirname, "..", "fixtures", `fake-prior-${name}.sh`), {
    encoding: "utf8",
  });
}
...
describe("스펙 §6 오류 처리", () => {
  ...
  it("③ warnings 가 있으면 배너를 낸다", () => {
    const q: Queue = JSON.parse(fixture("warnings"));
...
describe("빈 상태는 고장처럼 보이지 않는다", () => {
  it("큐가 셋 다 비어도 상태가 채워진다", () => {
    const q: Queue = JSON.parse(fixture("ok"));
...
describe("가짜 prior 픽스처", () => {
  ...
  it("JSON 을 내는 픽스처는 큐 계약을 지킨다", () => {
    for (const name of ["ok", "warnings", "onebroken"]) {
      const q = JSON.parse(fixture(name)) as Queue;
  ...
  it("깨진 JSON 픽스처는 정말로 깨져 있다", () => {
    expect(() => JSON.parse(fixture("broken-json"))).toThrow();
  });
});
