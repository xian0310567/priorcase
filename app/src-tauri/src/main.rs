...
fn main() {
    tauri::Builder::default()
        ...
        // ── 창을 닫아도 앱은 안 죽는다 ────────────────────────────────
        //
        // **이건 메뉴바 상주 앱이다.** 창은 트레이를 눌렀을 때 나타나는 표면일
        // 뿐이고, 앱의 본체는 뒤에서 계속 돈다 — 트레이 제목이 미확인 건수를
        // 보여 주고, 사람이 아무 때나 눌러 볼 수 있어야 한다.
        //
        // 기본 동작은 그 반대다: 창을 닫으면 창이 파괴되고, 마지막 창이 사라지면
        // Tauri 가 앱을 끝낸다. 그러면 사용자가 창을 닫는 순간 트레이 아이콘까지
        // 사라진다 — 앱을 껐다고 생각한 적이 없는데 꺼진다.
        //
        // 닫기를 막고 **숨긴다.** 진짜 종료는 트레이 메뉴의 "종료" 하나뿐이고,
        // 그것만 app.exit(0) 을 부른다. 문이 하나면 실수로 닫히지 않는다.
