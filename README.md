...
체크포인트 이후 구간
  ...
  → 키워드 시그널 ([capture] signals)
  → 같은 날 같은 도메인에 결정 노트가 **없을 것**
  → pending 기록
```
...
> 날짜 단위인 이유: 데몬과 기록 경로가 세션 식별자를 공유하지 않는다. MCP 의 세션 id 는
> SDK 가 만든 것이고 transcript 의 `sessionId` 와 다르다. 훅은 진짜 `session_id` 를
> 받으므로 Plan 4 에서 `source_session` 대조로 정밀해진다.
> **한계: 같은 날 같은 도메인의 두 번째 결정은 표시되지 않는다.**
