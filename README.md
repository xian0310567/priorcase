...
**면제는 소모성이다. 결정 노트 하나가 구간 하나를 면제한다.**

```
count = 이 구간을 가려 줄 수 있는 결정 노트 수
        (그 세션 id 를 단 노트) ∪ (구간이 걸친 날짜 + 같은 도메인 노트)

면제한다 ⟺ count > 지난번에 소모한 수
```
...
>
> **남은 거칢**: 날짜 축은 구간이 걸친 날짜로 세므로, 세션이 여러 날에 걸치면
> 날짜 범위가 넓어질 때마다 크레딧을 새로 살 수 있다. 다만 앞서 표시한 구간이
> 그 때문에 사라지지는 않는다.
...
| 어디 | 무엇 |
|---|---|
| `state.json` 의 `checkpoints[].at` | 마지막으로 훑은 시각. **아무 일도 안 한 스캔도 남긴다** |
| `state.json` 의 `checkpoints[].credited` · `suppressed` | 소모한 크레딧 · 누적 억제 횟수 |
